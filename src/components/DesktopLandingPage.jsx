...
export default function DesktopLandingPage({ setActiveTab }) {
  ...
  return (
    <div className="bg-surface-container-lowest text-on-surface font-body-lg antialiased min-h-screen flex flex-col w-full">
      ...
      <nav className="bg-surface/80 dark:bg-surface/80 backdrop-blur-md fixed top-0 w-full border-b border-outline-variant/30 z-50">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => setActiveTab && setActiveTab('generator')}
              className="bg-primary text-on-primary font-body-sm text-body-sm px-4 py-2 rounded-lg hover:opacity-90 transition-opacity cursor-pointer"
            >
              Get Started
            </button>
          </div>
      ...
      <main className="pt-24 pb-16 flex-grow">
        {/* Hero Section */}
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-16 md:py-24 fade-in-up">
          <div className="text-center max-w-3xl mx-auto">
            <h1 className="font-display-lg text-display-lg text-on-surface mb-6 leading-tight">Cold DMs that don't sound cold.</h1>
            <p className="font-body-lg text-body-lg text-on-surface-variant mb-8 max-w-2xl mx-auto font-light">Analyze prospect profiles instantly and generate highly personalized outreach that actually gets replies.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 max-w-lg mx-auto bg-surface p-2 rounded-full border border-outline-variant/50 shadow-sm focus-within:ring-2 focus-within:ring-secondary-container transition-all">
              ...
              <button 
                onClick={() => setActiveTab && setActiveTab('generator')}
                className="bg-primary text-on-primary font-body-sm text-body-sm px-6 py-3 rounded-full hover:bg-primary-fixed-dim hover:text-on-primary-fixed transition-colors whitespace-nowrap w-full sm:w-auto cursor-pointer"
              >
                Analyze Profile
              </button>
            </div>
          ...
          <div className="mt-16 max-w-2xl mx-auto bg-surface-container-lowest border border-outline-variant/30 rounded-xl shadow-lg p-6 relative overflow-hidden">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="bg-surface p-4 rounded-lg border border-outline-variant/20 mt-4 relative">
                  <p className="font-body-sm text-body-sm text-on-surface-variant italic">"Hey Alex, loved your recent post on sustainable scaling. Noticed TechStart is hiring engineers - thought I'd share a resource we use to find pre-vetted senior devs. Open to a quick chat?"</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* How it Works */}
        <section className="bg-surface py-20 mt-12 fade-in-up">
        ...
        {/* Features Grid */}
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-24 fade-in-up" id="features">
          <h2 className="font-headline-lg text-headline-lg text-center mb-16">Powerful features for seamless outreach</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/30 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="font-body-sm text-body-sm text-on-surface-variant font-light">Upload a list and generate 100s of unique drafts instantly.</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-surface py-24 fade-in-up">
          <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              ...
              {/* SlideInto DM */}
              <div className="bg-surface-container-lowest p-6 rounded-xl border-2 border-secondary-container shadow-md relative">
        ...
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-24 fade-in-up" id="pricing">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {/* Basic */}
            <div className="bg-surface-container-lowest p-8 rounded-xl border border-outline-variant/30 flex flex-col">
              ...
              <button 
                onClick={() => setActiveTab && setActiveTab('generator')}
                className="w-full bg-surface text-on-surface border border-outline-variant/50 font-body-sm text-body-sm px-4 py-2 rounded-lg hover:bg-surface-container-low transition-colors cursor-pointer"
              >
                Choose Starter
              </button>
            </div>
            ...
            {/* Pro */}
            <div className="bg-surface-container-lowest p-8 rounded-xl border-2 border-primary shadow-lg relative flex flex-col transform md:-translate-y-4">
              ...
              <button 
                onClick={() => setActiveTab && setActiveTab('generator')}
                className="w-full bg-primary text-on-primary font-body-sm text-body-sm px-4 py-3 rounded-lg hover:bg-primary-fixed-dim transition-colors cursor-pointer"
              >
                Get Pro
              </button>
            </div>
        ...
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-24 fade-in-up">
          <div className="bg-primary-container text-on-primary-container rounded-2xl p-12 text-center max-w-4xl mx-auto shadow-lg relative overflow-hidden">
            ...
            <p className="font-body-lg text-body-lg mb-8 opacity-90 max-w-2xl mx-auto relative z-10">Start sending highly personalized outreach today. No credit card required for your first 50 DMs.</p>
            <button 
              onClick={() => setActiveTab && setActiveTab('generator')}
              className="bg-on-primary text-primary font-title-md text-title-md px-8 py-4 rounded-xl hover:bg-surface-container-lowest transition-colors relative z-10 shadow-sm cursor-pointer"
            >
              Start Free Trial
            </button>
          </div>
